...
// Note: be sure to pop off all of the remaining CGEvents in the server queue.
static void _ReceiveEvent(CFMachPortRef port, void *msg, CFIndex size, void *info) {
    CGEventRef nextEventRef;
    while((nextEventRef = CGEventCreateNextEvent((int64_t)info)) != NULL) {
        GSEventRef gsEvent = GSEventCreateWithCGEvent(NULL, nextEventRef);
        if (gsEvent == NULL) continue;
